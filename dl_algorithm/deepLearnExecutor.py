...
from dl_algorithm.capsule_model import CapsuleModel
from trick.init_model import init_network
from trick.early_stop import EarlyStopping
from assis.common import get_time_dif
from config import DEVICE


class DeepLearnExecutor:
    def __init__(self, dl_config):
        self.dl_config = dl_config

    def judge_model(self, assign_path=""):
        if self.dl_config.model_name not in DL_MODEL_NAME:
            print("dl model name is not support, please see DL_MODEL_NAME of config.py")
        if self.dl_config.model_name == "lstm":
            self.model = LSTM(self.dl_config)
        elif self.dl_config.model_name == "cnn":
            self.model = TextCNN(self.dl_config)
        elif self.dl_config.model_name == "transformer":
            self.model = TransformerModel(self.dl_config)
        elif self.dl_config.model_name == "capsule":
            self.model = CapsuleModel(self.dl_config)
        # ! 其他模型
        else:
            pass
        init_network(self.model)
        print("初始化网络权重完成，默认采用xavier")
        self.model.to(self.dl_config.device)

    def train(
        self, train_loader, test_loader, dev_loader, model_saved_path, model_name
    ):
        # 设置优化器
        optimizer = torch.optim.AdamW(
            self.model.parameters(), lr=self.dl_config.learning_rate
        )
        best_test_f1 = 0
        ...
        # 学习更新策略--预热(warmup)
        if self.dl_config.update_lr:
            from transformers import get_linear_schedule_with_warmup

            num_warmup_steps = int(
                self.dl_config.warmup_prop * self.dl_config.epochs * len(train_loader)
            )
            num_training_steps = int(self.dl_config.epochs * len(train_loader))
        ...
        for epoch in range(self.dl_config.epochs):
            ...
            for index, data in enumerate(tqdm(train_loader, ncols=100)):
                pred = self.model(data["input_ids"].to(self.dl_config.device))
                loss = self.dl_config.loss_fct(
                    pred, data["label"].to(self.dl_config.device)
                ).mean()
                # 反向传播
                loss.backward()
            ...
            if (epoch + 1) % VERBOSE == 0:
                ...
                tqdm.write(
                    f"Epoch {epoch + 1:02d}/{self.dl_config.epochs:02d} \t time={elapsed_time} \t"
                    f"loss={avg_loss:.3f}\t lr={lr:.1e}",
                    end="\t",
                )

                if (epoch + 1 >= first_epoch_eval) or (
                    epoch + 1 == self.dl_config.epochs
                ):
    ...
    def evaluate(self, test_loader):
        ...
        with torch.no_grad():
            for test_data in test_loader:
                pred = self.model(test_data["input_ids"].to(self.dl_config.device))
                test_loss = self.dl_config.loss_fct(
                    pred, test_data["label"].to(self.dl_config.device)
                ).mean()
        ...
        if (
            self.dl_config.loss_type == "multi"
            or self.dl_config.loss_type == "marginLoss"
        ):
            multi = True
        else:
            multi = False
    ...
    def load_model(self, path, name):
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                self.model.load_state_dict(
                    torch.load(output_path, map_location=torch.device("cpu"))
                )
            self.model.eval()
            print("model 已加载预训练参数")
        except Exception as ex:
            print("model load error, ex={}", ex)
