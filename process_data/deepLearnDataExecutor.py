...
from trick.collate_padding import CollatePadding
...
class DataSetProcess:
    def __init__(self, train_data_path="", test_data_path="", dev_data_path=""):
        ...
        self.train_data, self.l1 = (
            self.load_data(self.train_data_path) if self.train_data_path else [[], []]
        )
        self.test_data, self.l2 = (
            self.load_data(self.test_data_path) if self.test_data_path else [[], []]
        )
        self.dev_data, self.l3 = (
            self.load_data(self.dev_data_path) if self.dev_data_path else [[], []]
        )

    def load_data(self, data_format):
        """
        默认是处理csv文件，其他形式的要改，csv的话文本内容要改成提供的demo格式
        增加list数据格式
        """
        if type(data_format) == str and data_format.endswith("csv"):
            df = pd.read_csv(data_format, encoding="utf-8")
...
class DeepLearnDataset(Dataset):
    """自定义torch的dataset"""
    ...
    def __getitem__(self, index):
        if self.label:
            return {"input_ids": self.data[index], "label": self.label[index]}
        else:
...
class DeepLearnDataExecutor:
    def __init__(self):
        pass

    def process(
        self, batch_size, train_data_path="", test_data_path="", dev_data_path=""
    ):
...
if __name__ == "__main__":
    d = DeepLearnDataExecutor()
    print(d.process(50, train_data_path="data/dl_data/train.csv"))
